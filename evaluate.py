...
def compute_oam_purity(field, target_l, radius=0.5, num_points=128):
    ...
    real_ring = map_coordinates(
        np.real(field), [y, x], order=1, mode="constant", cval=0.0
    )
    imag_ring = map_coordinates(
        np.imag(field), [y, x], order=1, mode="constant", cval=0.0
    )
...
if __name__ == "__main__":
    ...
    compute_metrics(
        model, ood_loader, device, radius=args.radius, num_points=args.num_points
    )
